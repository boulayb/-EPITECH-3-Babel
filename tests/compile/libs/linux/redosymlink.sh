...
rm -f libboost_system.so libboost_serialization.so libopus.so libportaudio.so libQtCore.so libQtCore.so.4 libQtCore.so.4.8 libQtGui.so libQtGui.so.4 libQtGui.so.4.8
